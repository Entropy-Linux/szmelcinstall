...
class AbstractCurses[ValueT](metaclass=ABCMeta):
	...
	def help_text(self) -> str:
		header = None
		if hasattr(Tui, '_t') and Tui._t is not None and Tui._t.global_header:
			header = Tui._t.global_header

		return header or 'Entropy Linux Installer'

	def info_text(self) -> str:
		info_locations = [
			Path(__file__).resolve().parents[2] / 'INFO.md',
		...
		return 'INFO.md not found.'

	def _show_help(self) -> None:
		help_text = Help.get_help_text()
		lines = help_text.split('\n')
		wrapped = self._wrap_lines(lines, self._help_window.width - 4)

		entries = [ViewportEntry('', 0, 0, STYLE.NORMAL)]
		entries += [ViewportEntry(f'  {e}', idx + 1, 0, STYLE.NORMAL) for idx, e in enumerate(wrapped)]
		self._help_window.update(entries, 0)

	def _show_info(self) -> None:
		info_lines = self.info_text().split('\n')
		wrapped = self._wrap_lines(info_lines, self._help_window.width - 4)
		entries = [ViewportEntry('', 0, 0, STYLE.NORMAL)]
		entries += [ViewportEntry(f'  {e}', idx + 1, 0, STYLE.NORMAL) for idx, e in enumerate(wrapped)]
		self._help_window.update(entries, 0)
...
class SelectMenu[ValueT](AbstractCurses[ValueT]):
	...
	def _draw(self) -> None:
		...
		if self._title_vp:
			title_entries = []
			title_entries.append(ViewportEntry(self.help_text(), 0, 0, STYLE.GOLD))
			keybinds = f'{tr("Press Ctrl+h for help")} | Ctrl+i for info'
			title_entries.append(ViewportEntry(keybinds, 1, 0, STYLE.NORMAL))

			self._update_viewport(self._title_vp, title_entries)
	...
	def _item_to_vp_entry(self, items: list[list[MenuItem]]) -> list[ViewportEntry]:
		...
		for row_idx, row in enumerate(items):
			...
			for col_idx, cell in enumerate(row):
				cur_text = ''
				style = cell.style if getattr(cell, 'style', None) else STYLE.NORMAL
...
class Tui:
	...
	def _set_up_colors(self) -> None:
		...
		curses.init_pair(STYLE.GOLD.value, curses.COLOR_YELLOW, curses.COLOR_BLACK)
